                  ...
                  {filteredPlayers.map(player => {
                    ...
                    return (
                      <button key={player.id} type="button" onClick={() => openPlayer(player)} className="group rounded-xl border border-gray-200 bg-white p-4 text-left transition hover:-translate-y-0.5 hover:border-gray-300 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[--primary-green]">
                        ...
                        {player.current_package && <p className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${player.current_package.computed_status === 'renewal_due' ? 'bg-amber-100 text-amber-800' : player.current_package.computed_status === 'expired' ? 'bg-rose-100 text-rose-800' : 'bg-violet-100 text-violet-800'}`}>↻ {player.current_package.package_name} · renews {formatDate(player.current_package.renewal_date).replace(/ \d{4}$/, '')}</p>}
        ...
        <Modal onClose={closePlayerPanel} labelledBy="player-detail-title">
          <div className="p-5 sm:p-6">
            ...
            {detailLoading ? <div className="py-12 text-center text-sm text-gray-500">Loading complete player record…</div> : <>
              ...
              <section className="mt-5 border-t border-gray-100 pt-5" aria-labelledby="player-renewal-title"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 id="player-renewal-title" className="text-sm font-bold text-gray-900">Package & renewal</h3><p className="mt-0.5 text-xs leading-5 text-gray-500">Programme-linked enrolment history. Renewal planning is coach-controlled and never creates a payment.</p></div><a href="/renewals" className="rounded-lg border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-bold text-violet-800 transition hover:bg-violet-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">Manage renewals</a></div>{selectedPlayer.package_enrolments?.length ? <div className="mt-3 space-y-2">{selectedPlayer.package_enrolments.map(enrolment => { const status = enrolment.computed_status || enrolment.status; const classes = status === 'renewal_due' ? 'bg-amber-100 text-amber-800' : status === 'expired' ? 'bg-rose-100 text-rose-800' : status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'; return <article key={enrolment.id} className="rounded-lg border border-violet-100 bg-violet-50/50 p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold text-gray-800">{enrolment.package_name}</p><p className="mt-0.5 text-xs text-gray-500">{enrolment.programme_name || 'No linked Programme'} · {formatDate(enrolment.start_date)} to {formatDate(enrolment.renewal_date)}</p>{enrolment.price_reference != null && <p className="mt-1 text-xs font-semibold text-violet-700">{Number(enrolment.duration_days)} days · {new Intl.NumberFormat('en-GB', { style: 'currency', currency: 'GBP' }).format(Number(enrolment.price_reference))} reference</p>}</div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${classes}`}>{String(status).replaceAll('_', ' ')}</span></div></article>; })}</div> : <p className="mt-2 text-sm text-gray-500">No package enrolment is recorded yet. Add one from the Packages & renewals workspace.</p>}</section>
